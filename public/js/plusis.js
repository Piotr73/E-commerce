...
function cargar_formulario(arg){
   ...
   if(arg == 4){var miurl=urlraiz+"/form_nuevo_producto"; }
...
$(document).on("submit",".formentrada",function(e){
  ...
  if(quien=="upload"){  var varurl=$(this).attr("action");  var div_resul="capa_formularios";  }
...
function asignar_rol(idusu){
   ...
   var miurl=urlraiz+"/asignar_rol/"+idusu+"/"+idrol+""; 
    ...
    { 
      var etiquetas="";
      for(i=0; i<data['rolessi'].length;i++){
          etiquetas+= '<span class="label label-warning">'+data['rolessi'][i].name+'</span> ';
      }
      $("#zona_etiquetas_roles").html(etiquetas);
      $("#rol1").html('');
      $("#rol2").html('');
      for(i=0; i<data['rolesno'].length;i++){
          var fila = '<option value="'+data['rolesno'][i].id+'">'+data['rolesno'][i].name+'</option>';
          $("select#rol1").append(fila);
      } 
      for(i=0; i<data['rolessi'].length;i++){
          var fila = '<option value="'+data['rolessi'][i].id+'">'+data['rolessi'][i].name+'</option>';
          $("select#rol2").append(fila);
      } 
    }).fail( function() 
    {
